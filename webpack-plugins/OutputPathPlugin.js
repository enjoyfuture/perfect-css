  ...
  apply(compiler) {
    ...
    compiler.plugin('done', (stats) => {
      Object.keys(entry).forEach((key) => {
        ...
        const currentMapFilename = sourceMapFilename.replace(/\[[\w-]+]/, currentFilename);

        const value = entry[key];
        // 相对路径
        let relativePath = __dirname;
        relativePath = relativePath.substring(0, relativePath.length - 'webpack-plugins'.length);
        ...
        if (paths.length > 0) {
          ...
          const moveFile = function (err) {
            if (!err) {
              // 移动文件
              mv(path.join(outputPath, currentFilename), path.join(targetPath, currentFilename));
              // 移动 Map 文件
              mv(path.join(outputPath, currentMapFilename), path.join(targetPath, currentMapFilename));
            }
          };
