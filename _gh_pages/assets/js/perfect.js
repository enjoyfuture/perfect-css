...
var ListMenu = function (_Component) {
  ...
  function ListMenu(element, config) {
    ...
    _this.handleItemScroll = function (evt) {
      ...
      if (upDown === 'up' && y < 0) {
        _this.itemsContainer.style.transform = 'translateY(' + Math.min(y + step, 0) + 'px)';
      } else if (upDown === 'down' && Math.abs(y) < _this.maxOffset) {
        _this.itemsContainer.style.transform = 'translateY(' + Math.max(y - step, -_this.maxOffset) + 'px)';
      }
    };
